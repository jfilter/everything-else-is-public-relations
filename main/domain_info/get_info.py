...
def get_info(url):
    ...
    website_title = title_element[0].text_content().strip() if len(title_element) > 0 else ''
    meta_description_element = doc.xpath("//meta[@name='description']/@content")
    website_meta_description = meta_description_element[0].strip() if len(meta_description_element) > 0 else ''

    return {'title': website_title, 'description': website_meta_description, 'reddits_per_day': get_reddits_per_day(url)}
